...
function statusDot(status: string): string {
  if (status === 'paid' || status === 'signed' || status === 'completed')
    return 'bg-[var(--success)]';
  if (status === 'failed') return 'bg-[var(--urgent)]';
  if (status === 'pending' || status === 'due' || status === 'awaiting')
    return 'bg-[var(--cat-billing)]';
    ...
    <section className="ops-section-card bg-[var(--surface-rail)]">
      ...
      <ul className="mt-3 divide-y divide-[var(--border-subtle)]">
        {items.map((item) => (
          <li
            key={item.key}
            className="flex items-center gap-2.5 py-2 text-sm first:pt-0 last:pb-0"
          >
            <span
              className={`size-2 shrink-0 rounded-full ${statusDot(item.status)}`}
              aria-hidden
            />
            <span className="min-w-[5.5rem] font-medium text-[var(--text-primary)]">
              {item.key}
            </span>
