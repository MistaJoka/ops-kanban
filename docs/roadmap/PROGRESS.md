...
`current_phase`: Optimistic background sync  
`current_task`: — (P13 shipped v0.5.0)  
...
1. Apply migration **017** (`column_entered_at`) via `npm run db:migrate` if not yet applied  
2. Run full regression: `npm run test:regression` after migrations applied  
3. Deploy v0.5.0 to Vercel staging with `DISABLE_AUTH=false`  
4. Pilot UAT on rapid drag/reorder, instant panel open, and background money sync  
...
**Optimistic background sync (P13) — v0.5.0 shipped.** Domain `outboundSyncQueue` with per-card FIFO, PATCH coalesce, and retry; `useOutboundSync` hook; board/panel/money fire-and-forget mutations; instant panel stub from board cache; sync pill queue depth + Retry. UNIT-SYNC-* + E2E-SYNC-*.
